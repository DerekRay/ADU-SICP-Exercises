...
;;;                         Code file GAME.SCM
;;;
;;; ... by Pezaris and Yanco, ADU SICP October 2000. Changes here were
;;; not significant.  Original code taken from the MIT Press web site,
...
(define (make&install-id-card name birthplace id)
  (let ((card (make-id-card name birthplace id)))
    (ask card 'install)
    card))

(define (make-id-card name birthplace idnumber)
  (let ((id idnumber)
        (thing (make-thing name birthplace)))
    (lambda (message)
      (cond ((eq? message 'id-card?) (lambda (self) true))
            ((eq? message 'id) (lambda (self) id))
            (else (get-method thing message))))))

(define (copy-id-card card)
  (let ((name   (symbol-append 'copy-of- (ask card 'name)))
        (place  (ask card 'place))
        (id     (ask card 'id)))
    (make&install-id-card name place id)))
